...
> 🩺 **配置自检**：系统管理 → 证书提供商（CA）→ 每个厂商下方的「验证配置」，
> 用于在不申请证书、不消耗签发配额的前提下判断凭据是否可用。
> 校验顺序为：字段完整性 → 账户私钥 PEM 解析 → HMAC Key base64 解码
> → 向 CA 的 `newAccount` 发起一次只读探测（`onlyReturnExisting`）。
> 前两项为本地校验，可离线发现绝大多数填写错误（如 PEM 缺换行、复制不全）。
>
> ⚠️ **只读探测的能力边界**（三家 CA 行为实测不同，不要过度解读结果）：
> - **SSL.com** 在只读模式下也会校验 EAB，因此能直接判定凭据对错；
> - **GTS / ZeroSSL** 会忽略只读请求中的 EAB，凭据无论对错都只返回
>   `accountDoesNotExist`。此时自检会以 `!` 标记「未能验证」而非「通过」，
>   仅能确认目录可达、私钥可用、字段格式合法。
>
> 如需完整校验 GTS / ZeroSSL 的 EAB，可勾选「注册账户以完整校验 EAB」。
> 注册 ACME 账户**不消耗证书签发配额**（签发配额只在 `newOrder` 时消耗），
> 但 **GTS 的 EAB 是一次性凭据**：注册成功后该 EAB 即失效，日后再更换
> `GTS_KeyTS` 就需要重新申请 EAB。因此该选项默认关闭，仅在首次配置时使用。
